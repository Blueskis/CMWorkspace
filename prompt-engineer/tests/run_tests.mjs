#!/usr/bin/env node
// Extracts the logic block from prompt-engineer.html and asserts against it.
...
const htmlPath = path.join(__dirname, "..", "prompt-engineer.html");
...
if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx) {
  console.error("Could not find LOGIC START/END markers in prompt-engineer.html");
  process.exit(1);
}
const logicCode = html.slice(startIdx, endIdx);

const context = vm.createContext({ console });
vm.runInContext(logicCode, context, { filename: "logic-block.js" });
const PE = context.PE;

if (!PE) {
  console.error("Logic block did not expose a global `PE` object.");
  process.exit(1);
}
...
function answerAll(archetypeId, value = (id) => `answer for ${id}`) {
...
test("16. same inputs twice -> byte-identical output", () => {
  ...
  const { answers } = answerAll("analyze");
...
test("18. Claude follow-up answers append to Context and Constraints, never replace", () => {
  ...
  const { answers } = answerAll("summarize");

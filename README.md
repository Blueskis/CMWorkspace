...
| `deck-builder` | **v0.1 (MVP)** — a client's brand + approved template + source documents → a first-draft consulting deck (steerco update, case for change, findings & recommendations, and six more types), built on a shared core with the published Training Deck Generator artifact |
...
| `short_form_video` | scene spec + captions | ElevenLabs MCP (narration only; picture has no producer) | planned, v0.3 |
| `explainer_video` | scene spec + captions | ElevenLabs MCP (narration only; picture has no producer) | planned, v0.3 |
...
Built on a shared core, `lib/deck/`, moved out of `webapp/` — the `training-material-generator`
skill's browser artifact, now re-skinned to this project's house style (see CLAUDE.md)
and published as **Training Deck Generator**, listed in CLAUDE.md's published-artifacts
table — so the same template profiler, layout mapper, text-fit math, and pptx assembler
serve both the skill and that artifact rather than existing twice. `lib/deck_index.py` and

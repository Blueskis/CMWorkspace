...
tools/proposal-development/  # browser tool: tender in -> correct the read -> shortlist ->
                              # first-draft .pptx out, built on the firm's own template —
                              # see its own README
```

Scripts are stdlib-only and each runs standalone with `--help`.

## CM Proposal Reference Tool (browser)

A published claude.ai Artifact (not part of the plugin above) that carries the same
provenance and coverage invariants into an interactive tool: read a tender, correct the
automatic read with direct controls or by asking Claude, shortlist past proposals against
it from Airtable, then draft and export a first-version `.pptx` built on the firm's own
PowerPoint template. See `tools/proposal-development/README.md`.
